...
- [0004 SMTP digest transport, gating, and test layers](0004-smtp-digest-transport.md): smtplib behind a Notifier seam; env-only credentials (repr-safe), default port 1025 targets a local sink so real sends are owner-deliberate; four test layers. (proposed)
