...
- `.codex/` and `AGENTS.md` — the same workflow for the Codex CLI (committed 2026-07-11). The hook scripts under `.codex/hooks/` are byte-identical mirrors of the `.claude` ones, declared in the `mirrors:` list in `docs/okf-map.yml` so the kit updater syncs them on upgrades; keep the two playbooks (`CLAUDE.md`/`AGENTS.md`) telling the same story.
